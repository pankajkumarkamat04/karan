...
export class UserRepository {
  ...
  static async createSuAdminUser({ username, email, password }) {
    try {
      ...
      const user = await prisma.user.create({
        data: {
          first_name: username, // Using first_name instead of username
          email: email,
          password: password,
          type: 'su_admin',
          status: 1,
  ...
  static async inviteUser({
    ...
    try {
      const user = await prisma.user.create({
        data: {
          first_name: name,  // Using first_name instead of name
          email: email,
          type: role_id,    // Using type field for role
          status: 1
        },
      });
  ...
  static async attachRole({
    ...
    // Since we don't have roleUser table, we can update the user's role directly
    const user = await prisma.user.update({
      where: { id: user_id },
      data: {
        type: role_id, // Using the type field to store role
      },
    });
    return user;
  }
